      ...
      <section className="container mx-auto px-4">
        {banners.length > 0 ? (
          <div className="relative h-[500px] w-full rounded-3xl overflow-hidden group">
            <img src={banners[0].image} alt={banners[0].title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
            <div className="absolute inset-0 bg-black/50 flex flex-col justify-center px-8 md:px-16 text-white">
              ...
              {banners[0].link && (
                <Link href={banners[0].link} className="bg-primary text-primary-foreground px-6 py-3 rounded-full font-semibold w-fit flex items-center gap-2 hover:opacity-90 transition">
          ...
          {products.map((product) => (
            <div key={product.id} className="group border border-border bg-card rounded-2xl overflow-hidden hover:shadow-lg transition">
              <div className="aspect-video relative overflow-hidden">
                <img src={product.image || '/placeholder-product.png'} alt={product.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xl font-bold text-primary">${product.price.toString()}</span>
                  <Link href={`/products/${product.slug}`} className="text-sm bg-secondary hover:bg-primary hover:text-primary-foreground px-3 py-1.5 rounded-lg transition">Details</Link>
                </div>
              </div>
            </div>
          ))}
